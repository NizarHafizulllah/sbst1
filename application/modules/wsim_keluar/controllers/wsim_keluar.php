<?php
class wsim_keluar extends wilayah_controller {
	...
	function index(){
		...
		$data_array = array();
        $data_array['arr_polres'] = $this->cm->arr_dropdown('m_polres', 'id', 'polres', 'polres');
		$data_array['arr_jenis'] = $this->cm->arr_dropdown('sim_jenis', 'id', 'nama', 'nama');

		$content = $this->load->view($this->controller."_view",$data_array,true);

		$this->set_subtitle("SIM - Material Keluar");
		$this->set_title("SBST");
    ...
    if($this->form_validation->run() == TRUE ) { 
        ...
        if (!empty($data)) {
            $post['jumlah'] = $data['jumlah']-$post['keluar'];
        ...
        if($res){
            ...
            $arr = array("error"=>true,'message'=>"Material Ini Tidak Pernah Masuk");
        }
